...
struct YouziModelTableTests {
    ...
    func scenarioSource() throws {
        ...
        let panelStart = try #require(source.range(of: "private var panel: some View {"))
        let panelEnd = try #require(source.range(of: "private func openModelSettings()"))
        let panel = source[panelStart.lowerBound..<panelEnd.lowerBound]
        let tabs = try #require(panel.range(of: "YouziScenarioModelToolbar("))
        let models = try #require(panel.range(of: "ScrollView {"))
        let scrollEnd = try #require(panel.range(of: "}.frame(maxHeight: 240)"))
        let memory = try #require(panel.range(of: "YouziScenarioModelMemoryFooter(occupancy: occupancy, refreshing: loading)"))
        #expect(tabs.upperBound < models.lowerBound, "Scenario tabs stay above the model list")
        #expect(scrollEnd.upperBound < memory.lowerBound, "Memory stays below and outside the scrolling list")
