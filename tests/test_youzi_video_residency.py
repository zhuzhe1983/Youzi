...
def engines():
    ...
    registry.add(
        ModelEntry(engine=chat, model_name="chat", model_path="chat"), is_default=True
    )
    registry.add(
        ModelEntry(
            engine=movie,
            model_name="video",
            model_path="local/video",
            aliases={"movie"},
        )
    )
...
async def test_video_lease_prevents_eviction_without_changing_chat(monkeypatch):
    ...
    manager = ResidentModelManager(
        registry=registry, loader=load, memory_limit_bytes=10**9
    )
    manager.register_primary(registry.get_entry("chat"), estimated_bytes=100)
    await manager.load("video", estimated_bytes=100)
    monkeypatch.setattr(
        config, "get_config", lambda: SimpleNamespace(residency_manager=manager)
    )
    async with video._video_engine_lease(movie):
        assert (
            next(m for m in manager.snapshot()["models"] if m["id"] == "video")[
                "active_requests"
            ]
            == 1
        )
...
async def test_dynamic_video_load_keeps_primary_and_checks_runtime(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        video_lane, "require_video_runtime_or_exit", lambda model: calls.append(model)
    )
    monkeypatch.setattr(
        video_lane,
        "VideoEngine",
    ...
    entry = await server._load_dynamic_resident_model(
        "Anes1032/Wan2.2-I2V-A14B-mlx-q8", None
    )
