...
def lanes(monkeypatch):
    ...
    def run(lane, model, op, func, *args, **kwargs):
        ...
        rows[lane] = dict(lane=lane, model=model if op != "unload" else None,
    ...
    monkeypatch.setattr(worker_module.audio_worker, "snapshot", lambda: list(rows.values()))
...
@pytest.mark.asyncio
async def test_preload_both_audio_lanes_preserves_chat_image_and_reuses_weights(lanes, monkeypatch):
    rows, tts, stt = lanes
    cfg = get_config()
...
def test_preload_control_plane_requires_key_in_anonymous_mode(lanes, monkeypatch):
    ...
    assert client.post("/v1/audio/models/load", json={"model": "kokoro"}).status_code == 401
    response = client.post("/v1/audio/models/load", json={"model": "kokoro"},
...
def test_discovery_includes_only_materialized_audio_lanes(lanes, monkeypatch):
    rows, _, _ = lanes
    cfg = get_config()
    for name, value in dict(ready=True, draining=False, engine=None,
                            model_registry=None, embedding_engine=None,
                            residency_manager=None).items():
        monkeypatch.setattr(cfg, name, value)
    rows["tts"] = dict(lane="tts", model="mlx-community/Kokoro-82M-bf16",
                       state="resident", loaded_at=1)
    assert set(models._loaded_model_ids()) == {"kokoro", "mlx-community/Kokoro-82M-bf16"}
    rows["tts"]["state"] = "busy"
...
def test_preflight_accepts_cached_audio_weight_formats(tmp_path, monkeypatch, extension):
...
def test_preflight_rejects_incomplete_cache_and_capacity(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(get_config(), "residency_manager", SimpleNamespace(
        snapshot=lambda: {"memory_available_bytes": 1}
    ))
    with pytest.raises(HTTPException) as exc:
        audio._check_audio_preload_capacity("local/audio")
    assert exc.value.status_code == 507
